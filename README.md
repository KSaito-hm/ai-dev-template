...
人間から新しいリポジトリのURLが伝えられているか確認してください。
...
### STEP 3：テンプレート構成を展開する（新規リポジトリの場合のみ）

リポジトリが空の場合（新規作成直後）は、このテンプレートの構成を展開します。

```bash
# ディレクトリ構成を作成
mkdir -p docs/ADR docs/assets src tests/fixtures scripts prompts

# 空フォルダ維持用
touch docs/assets/.gitkeep src/.gitkeep tests/fixtures/.gitkeep scripts/.gitkeep
...
続けて `docs/` と `prompts/` 配下の各ファイルをこのテンプレートの内容をもとに作成してください。
