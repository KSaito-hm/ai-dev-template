...
featureブランチの作業が完了しdevelopにマージしたら、**同じタイミングで**以下を実行すること。

1. ローカルのfeatureブランチを削除する（`git branch -d feature/xxx`）
2. リモートのfeatureブランチを削除する（`git push origin --delete feature/xxx`）
3. `git branch -a` で整理結果を確認し、develop / main のみであることを報告する

次のタスクに着手する前にこれを完了させること。人間に催促されてから行うのでは遅い。
...
仕様変更が発生したとき、以下の順序を**厳守**すること。コード実装を先に行ってはならない。

```
① docs/01_requirements.md   要件の変更を反映する
② docs/02_architecture.md   構成への影響を反映する
③ docs/03_design.md         詳細設計を更新する
④ docs/04_tasks.md          タスクのステータス・備考を更新する
⑤ 実装（コード変更）
⑥ 変更履歴の記録            各docsの変更履歴テーブルに日付・内容を追記する
```

**重要：** 動作確認フィードバックやバグ修正であっても、設計変更を伴う場合はドキュメント更新を先に行うこと。「軽微な修正だからコードだけ」は禁止。

### ブランチ整備の原則

featureブランチの作業が完了したら、**以下を1セットとして必ず実行**すること。人間に催促される前に自発的に行うこと。
...
⑤ git branch -a で整理結果を確認する
```

**重要：** 次のタスクに着手する前に、前のタスクのブランチ整備を完了させること。ブランチが散乱した状態で作業を続けてはならない。

### コミットの原則

1つのコミットには**ドキュメント更新とコード変更の両方を含める**か、**ドキュメント更新を先にコミット**してからコード変更をコミットすること。コード変更だけのコミットを先に行い、ドキュメント更新を後追いにしてはならない。
...
- ドキュメントを更新せずに実装を進める
- 人間の確認なしに複数ファイルを変更する
- featureブランチを整理せずに次のタスクに着手する
- 変更履歴を記録せずにドキュメントを更新する
- 動作確認フィードバックの修正をドキュメント更新なしに行う
